...
export function AnnouncementArticleClient({
  ...
  const status: EditStatus = isSaving || isPublishing || isDeleting
    ? "saving"
    : hasChanges
      ? "dirty"
      : "saved"
  const statusLabel = isDeleting
    ? "刪除中…"
    : isPublishing
      ? announcement.status === "published" ? "取消發布中…" : "發布中…"
      : isSaving
        ? "儲存中…"
        : hasChanges
          ? "尚未儲存"
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="mb-10 flex items-center justify-between gap-4">
        ...
        <ShareButtons url={`/announcement/${announcement.id}`} title={announcement.title} />
      </div>
