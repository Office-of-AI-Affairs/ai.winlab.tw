                      ...
                      {displayedAnnouncements.map((item) => (
                        <tr key={item.id} className="h-12 border-t border-border hover:bg-muted/60 transition-colors">
                          <td colSpan={isAdmin ? 4 : 3} className="p-0">
                            <Link
                              href={(() => {
                                const announcementSlug = encodeURIComponent(item.slug || item.id);
                                return isAdmin
                                  ? `/events/${slug}/announcements/${announcementSlug}${item.status === "draft" ? "?mode=edit" : ""}`
                                  : `/events/${slug}/announcements/${announcementSlug}`;
                              })()}
