...
describe("sitemap contracts", () => {
  ...
  test("sitemap includes public profile, announcement, and event result routes, but not team routes", () => {
    ...
    assert.ok(sitemapFile.includes('`${BASE_URL}/events/${eventSlugMap[announcement.event_id]}/announcements/${encodeURIComponent(announcement.slug)}`'))
    assert.ok(sitemapFile.includes('`${BASE_URL}/announcement/${encodeURIComponent(announcement.slug)}`'))
