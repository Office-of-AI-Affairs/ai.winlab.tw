 ...
 * source attribution for the 429/401 signals oauth/callback already emits,
 * and every other request besides.
 *
 ...
 * two WARN logs in oauth/callback/route.ts don't have this problem — they
 * carry `client.address`/`geo.*` and the route (via the log message) in one
 * record.
 */
